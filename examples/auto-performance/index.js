...
class PerformanceManager {
	constructor() {
		...
		this.fpsHistory = [];
		this.FPS_HISTORY_LENGTH = 10;
		this.performanceAdjustmentCooldown = 0;
		this.PERFORMANCE_ADJUSTMENT_COOLDOWN = 60;
		this.consecutiveAdjustments = 0;
		this.MAX_CONSECUTIVE_ADJUSTMENTS = 5;
		this.emergencyMode = false;
		this.emergencyModeLogged = false;
	}
	...
	updateFPS() {
		...
		if (this.performanceAdjustmentCooldown > 0) {
			this.performanceAdjustmentCooldown--;
		}
	}
	...
	shouldAdjustQuality() {
		return this.performanceAdjustmentCooldown === 0 && 
			   this.consecutiveAdjustments < this.MAX_CONSECUTIVE_ADJUSTMENTS;
	}

	adjustmentMade() {
		this.performanceAdjustmentCooldown = this.PERFORMANCE_ADJUSTMENT_COOLDOWN;
...
function fluidSimulation({ pane, contextID, glslVersion}) {
	...
	// Mantener el canvas detrás de la UI (Tweakpane, modal, overlay)
	// Usamos z-index negativo para evitar que oculte controles.
	
	// Set canvas size and style before adding to DOM
	...
	// Init state.
	// Force a reflow to ensure canvas dimensions are calculated
	canvas.offsetWidth; // Force reflow
	
	const width = canvas.clientWidth || canvas.width || window.innerWidth || 1920;
	...
	const safeDimensions = [
		Number(velocityDimensions[0]) || 1,
		Number(velocityDimensions[1]) || 1
	];
	...
	function loop() {
		...
		// Auto-tuning logic
		if (PARAMS.autoTuning && performanceManager.shouldAdjustQuality()) {
			const avgFPS = performanceManager.getAverageFPS();
			
			if (avgFPS < 30 && !performanceManager.emergencyMode) {
				// Performance is poor, reduce quality
				const currentIndex = QUALITY_SEQUENCE.indexOf(currentQuality);
				if (currentIndex > 0) {
					currentQuality = QUALITY_SEQUENCE[currentIndex - 1];
					currentPreset = QUALITY_PRESETS[currentQuality];
					performanceManager.adjustmentMade();
				} else {
					performanceManager.emergencyMode = true;
					if (!performanceManager.emergencyModeLogged) {
						console.warn('Performance emergency mode activated - minimum quality reached');
						performanceManager.emergencyModeLogged = true;
					}
				}
			} else if (avgFPS > 50 && currentQuality !== 'high') {
				// Performance is good, try to increase quality
				const currentIndex = QUALITY_SEQUENCE.indexOf(currentQuality);
				if (currentIndex < QUALITY_SEQUENCE.length - 1) {
					currentQuality = QUALITY_SEQUENCE[currentIndex + 1];
					currentPreset = QUALITY_PRESETS[currentQuality];
					performanceManager.adjustmentMade();
				}
			...
			// Reset emergency mode if performance improves
			if (performanceManager.emergencyMode && avgFPS > 35) {
				performanceManager.emergencyMode = false;
				performanceManager.emergencyModeLogged = false;
				performanceManager.resetAdjustments();
			}
		}
	...
	ui.push(pane.addInput(PARAMS, 'autoTuning', { label: 'Auto Tuning' }));
	ui.push(pane.addInput(PARAMS, 'showFPS', { label: 'Show FPS' }));
...
function main({ pane, contextID, glslVersion }) {
	...
	setTimeout(() => {
		...
		if (exampleSelect) {
			exampleSelect.addEventListener('change', async function(e) {
				const selectedExample = e.target.value;
				
				try {
					// Use the same parameters that were passed to main
					const newResult = await switchExample(selectedExample, pane, contextID, glslVersion);
					
					// Update the global variables that wrapper.js expects
					if (newResult && typeof window !== 'undefined') {
						// Actualiza el estado del wrapper para usar el nuevo ejemplo
						window.currentExampleResult = newResult;
						if (window.applyExampleResult) window.applyExampleResult(newResult);
					}
				} catch (error) {
					console.error('Error switching example:', error);
				}
			});
		}
	}, 100);
	
	// Since switchExample is now async, we need to handle the initial result properly
	if (result instanceof Promise) {
		return result;
	} else {
		// For backward compatibility, wrap in a resolved promise
		return Promise.resolve(result);
	}
}

// Remove the DOMContentLoaded listener as it's now handled in main()
