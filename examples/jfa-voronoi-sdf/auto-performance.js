// JFA Voronoi + SDF module for the Auto-Performance page
// Exposes jfaVoronoiSdfMain which is referenced by examples.js

async function jfaVoronoiSdfMain({ pane, contextID, glslVersion }) {
  const performanceManager = new PerformanceManager();
  return jfaVoronoiSdfSimulation({ pane, contextID, glslVersion, performanceManager });
}

async function jfaVoronoiSdfSimulation({ pane, contextID, glslVersion, performanceManager }) {
  const {
    GPUComposer,
    GPUProgram,
    GPULayer,
  ...
  // Register Tweakpane Essentials for segmented controls, if available
  ...
  // Ensure non-zero pixel dimensions immediately to avoid zero-sized GPULayers
  ...
  // Inline shaders to keep this module isolated from auto-performance/shaders
  const jfaSeedSrc = `
...
`;
  ...
  // Responsive resize handler to keep layers and uniforms in sync
  function onResize() {
    ...
    // Keep local references in sync for interactivity
    pos = data.pos;
    attrs = data.attrs;
  ...
  try {
    ...
    // Fallback if essentials plugin not loaded: simple toggle
  ...
  // Auto-performance integration: change quality presets at runtime
  function updateQuality(quality) {
    const settings = QUALITY_PRESETS[quality];
    const scale = settings.particleDensity || 1;
    const base = Math.round((canvas.width * canvas.height) / (512 * 512) * 256);
    const newCount = Math.max(32, Math.min(1024, Math.round(base * scale)));
    PARAMS.seedCount = newCount;
    PARAMS.refineAfterJfa = quality === 'high' || quality === 'medium';
    seedCountCtrl.refresh();
    refineCtrl.refresh();
  ...
  function runJFA() {
    ...
    // Jump steps: largest power of two down to 1
    const maxDim = Math.max(canvas.width, canvas.height);
    let jump = 1;
    while (jump * 2 <= maxDim) jump *= 2;
  ...
  // Main loop
  function loop() {
    // Adjust dynamic params if FPS drops
  ...
  const activePointers = new Map(); // pointerId -> seedIndex
  ...
  function setSeedPosition(i, x, y) {
    // Update CPU arrays and upload to GPU
    pos[2 * i] = x;
    pos[2 * i + 1] = y;
    attrs[3 * i] = x;
    attrs[3 * i + 1] = y;
  ...
  return { loop, dispose, composer, canvas, updateQuality };
}